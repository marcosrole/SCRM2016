...
//                                        Yii::app()->controller->redirect(array ('/site/index'));
        ...
        {    
            ...
            $auth->clearAll();
            $auth->createOperation('createDispositivo','Crear dispositivo');
            // Create our operations
            $auth->createOperation('listDispositivo', 'Listar Dispositivos');
            $auth->createOperation('adminDispositivo', 'Administrar Dispositivos');
            
            $role = $auth->createRole('inspector', 'Usuario Inspector');
            $role->addChild('createDispositivo');
            
            $role = $auth->createRole('admin', 'Administrador');
            $role->addChild('inspector');
            $role->addChild('listDispositivo');
            $role->addChild('adminDispositivo');
            
            $auth->assign('inspector',89);
            $auth->assign('admin',88);
            
            echo "AuthItem Creado";
        }
	/**
