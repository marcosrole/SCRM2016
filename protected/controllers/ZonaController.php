...
class ZonaController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Zona']))
		{
                        $model->nombre=  strtoupper($_POST['Zona']['nombre']);
                        if(!Zona::model()->findByAttributes(array('nombre'=>$model{'nombre'}))){                            
                            if($model->save())
				$this->redirect('index');
                            Yii::app()->user->setFlash('success', "<strong>Zona almacenada!</strong>  ");
                        }else  Yii::app()->user->setFlash('error', "<strong>Error!</strong> Ya existe una zona con el mismo nombre");                        
		}
